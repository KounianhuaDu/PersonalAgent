...
if __name__ == "__main__":
    ...
    device = model.device
    for uid, vector_dataset in vector_dataset_all.items():
        ...
        if args.model_name == "gemma-2-9b" or args.model_name=="llama-3.1":
            output_dir = os.path.join(
                args.data_path, "caa_vector_pt", f"{args.model_name}_0.15"
            )
        else:
            output_dir = os.path.join(
        ...
        for layer in args.layers:
            ...
            if (len(all_pos_layer) == 0):
                p_activations = model.get_last_activations(layer)
                vec = torch.zeros(p_activations.shape[-1])
            else:
