...
def compress(layer, threshold, attn_mask, mlp_mask, attn_mean_inp, mlp_mean_inp, device, bias=True, unstr=False):
    ...
    if attn_mask is not None:
        ...
        head_dim = layer.self_attn.head_dim # 128
        num_heads = layer.self_attn.num_heads # 32
        retain_dims = torch.count_nonzero(temp_attn)
        # print(head_dim, num_heads, retain_dims, temp_attn.shape, layer.self_attn.num_key_value_heads)
        retain_head_dim = retain_dims // num_heads     
        
        attn_mask = attn_mask.reshape(num_heads, head_dim)  # Reshape to (num_heads, head_dim)
        _ , indices = torch.topk(attn_mask, retain_head_dim, dim=1)
        attn_q_mask = torch.zeros_like(attn_mask)
        attn_q_mask.scatter_(1, indices, 1)  # Set the top-k indices to 1
        attn_q_mask = attn_q_mask.flatten().bool()
        
        attn_mask = attn_mask.reshape(layer.self_attn.num_key_value_heads, -1, head_dim).mean(dim=1) # [8, 128]
        _ , indices = torch.topk(attn_mask, retain_head_dim, dim=1)
        attn_k_mask = torch.zeros_like(attn_mask)
        attn_k_mask.scatter_(1, indices, 1)  # Set the top-k indices to 1
        attn_k_mask = attn_k_mask.flatten().bool()
        
        # Prune the query, key and value projection weights
        # We reduce the size of the weights based on the attention mask
        '''print(attn_mask.shape)
        print(layer.self_attn.q_proj.weight.data.shape) # 4096，4096
        print(layer.self_attn.k_proj.weight.data.shape) # 1024 4096
        print(layer.self_attn.v_proj.weight.data.shape) # 1024，4096
        print(torch.where(attn_mask)[0])
        assert layer.self_attn.q_proj.weight.data.shape[0] >= attn_mask.shape[0]'''
        ...
        layer.self_attn.head_dim = retain_head_dim
        layer.self_attn.hidden_size = retain_head_dim * num_heads
        layer.self_attn.config.head_dim = retain_head_dim
        layer.self_attn.config.hidden_size = retain_head_dim * num_heads
        layer.self_attn.rotary_emb = LlamaRotaryEmbedding(config=layer.self_attn.config)
        
        if bias:
            # Re-initialize the Linear layer with new shape and bias
            layer.self_attn.o_proj.in_features = attn_q_mask.sum().item()
...
def prune_flap(base_model_addr, sparsity_ratio, calibration_data, save_model_path):
    ...
    attn_metric = attn_metric.reshape(len(layers), -1) # [32, 4096]
    ...
    compression_weight[indices < attn_metric.numel()] = 2.5 / 3
