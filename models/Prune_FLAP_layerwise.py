...
def prune_flap(base_model_addr, sparsity_ratio, calibration_data, save_model_path):
    ...
    for i in range(len(layers)):
        ...
        attn_metric = attn_metric.reshape(1, -1, 512).mean(dim=2)
    ...
    avg_metric = torch.softmax(avg_metric, dim=0)
    # max_sparsity = sparsity_ratio / (1 - avg_metric).mean()
    layer_sparsity = 1 - avg_metric / avg_metric.mean() * (1 - sparsity_ratio)# 每一层的稀疏度
    ...
    attn_metric = standarlization(attn_metric)
    attn_metric = attn_metric.reshape(len(layers), -1, 512).mean(dim=2)
