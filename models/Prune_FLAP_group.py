...
from .eval_c4 import eval_ppl_wikitext 
init(autoreset=True)
...
def compress(layer, threshold, attn_mask, mlp_mask, attn_mean_inp, mlp_mean_inp, device, bias=False, unstr=False):
    """
...
def prune_flap(base_model_addr, sparsity_ratio, calibration_data, save_model_path):
    ...
    model = AutoModelForCausalLM.from_pretrained(base_model_addr)
    ...
    nsamples = len(calibration_data['cluster_data'])
    print(calibration_data['cluster_data'][0])
    # exit()
    dataloader = []
    for line in tqdm(calibration_data['cluster_data']):
    ...
    # In higher versions of Transformers, position_embeddings need to be explicitly passed in. 
    position_embeddings = []
    for j in range(nsamples):
        inputs_embeds = model.model.embed_tokens(inps[j].long())
        position_embeddings.append(model.model.rotary_emb(inputs_embeds, position_ids))
    ...
    attn_metric = attn_metric.reshape(len(layers), -1, 512).mean(dim=2) # [32, 8]
    ...
    for idx in range(len(layers)):
        ...
        if f"model.layers.{idx}" in getattr(model, 'hf_device_map', {}): 
            compress(model.model.layers[idx], threshold, attn_metric[idx], None, attn_baseline_inp_list[idx], None, model.hf_device_map[f"model.layers.{idx}"], unstr=False,
                     bias=model.model.layers[idx].self_attn.o_proj.bias is not None)
        else:
        ...
        if f"model.layers.{idx}" in getattr(model, 'hf_device_map', {}): 
            compress(model.model.layers[idx], threshold, None, mlp_mask[idx], None, mlp_baseline_inp_list[idx], model.hf_device_map[f"model.layers.{idx}"], unstr=False,
                     bias=model.model.layers[idx].mlp.down_proj.bias is not None)
        else:
    ...
    # return post_tune(model, tokenizer, calibration_data['self_data'])  

def post_tune(model, tokenizer, train_data):
    train_dataset = []
    print(train_data[0])
